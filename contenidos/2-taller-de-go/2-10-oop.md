...
````{admonition} <code class="docutils literal notranslate"><span class="pre">stack/stack.go</span></code>
---
class: note
---
```go
...
En nuestro caso, para crear una variable de tipo `Stack`{l=go} podemos utilizar la forma literal de declarar una estructura. También es común crear un método llamado `NewStack`{l=go} que sirve como constructor y que devuelva un puntero a un variable de tipo `Stack`{l=go}, el cual puede recibir argumentos y asignar esos valores a los campos de la estructura.

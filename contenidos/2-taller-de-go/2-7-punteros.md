...
1. Escriba una función `swap` que intercambie los valores de dos variables enteras usando punteros.

2. Escriba una función `sumarPunteros` que reciba dos punteros a enteros (`*int`) y devuelva la suma de los valores apuntados. Si alguno es `nil`, debe tratarlo como `0`.

3. Escriba una función `dividir` que reciba un dividendo y un divisor (enteros), y devuelva el cociente como valor de retorno y el resto a través de un parámetro puntero `*int`.

4. Escriba una función `inicializarArreglo` que reciba un puntero a un arreglo `[5]int` y lo inicialice con los valores `1, 2, 3, 4, 5`.

5. Escriba una función `maximo` que reciba un puntero a un arreglo `[6]int` y devuelva el valor máximo del arreglo.
...
Los esqueletos de estos ejercicios están en `07-punteros/ejercicios/` del repositorio [taller-go](https://github.com/untref-ayp2/taller-go.git).

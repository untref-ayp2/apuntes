...
:::{admonition} Definición
$$
T(n) = O(f(n)) \; \textrm{si} \; \exists \; c, \; n_0 \; \textrm{tal que} \; T(n) \leq c \times f(n) \; \forall n > n_0
$$

$\textrm{donde } c \textrm{ es una constante positiva y } n_0 \textrm{ un número natural}$
:::
...
Por ejemplo, si tenemos la función $T(n)=5n + 1$, se verifica que es de la familia de $O(n)$, es decir la tasa de crecimiento de $T(n)$ está acotada por $f(n)=n$, ya que:

$\exists \; c = 5$ y $n_0 = 1$

tal que:

$5n \ge 4n+1 \; \forall n \ge 1$
